...
function moveBall() {
    ...
    if (parseInt(ting_ball_style.top) > 700) {  // ;공의 종료 시점
        ...
        bottomBlock.innerHTML += "<h2>Game Over</h2>";
        clearTimeout(coll_t);
    } else {
...
function collisionBall() {
    ...
    if (ballVerticalPos > 650) {
        if ((ballHorizonPos > barHorizonPos - 100) && (ballHorizonPos < barHorizonPos + 100)) {
            ...
            move_ver *= -1;
            score += 10;
            bottomBlock.innerHTML = "<h2> score: " + score + "</h2>";
        }
    }
...
window.onkeydown = function moveBar(evt) {
    ...
    switch (keyValue) {
        case 37:
            ...
            row_bar_style.left = (parseInt(row_bar_style.left) - 15) + "px";
            break;
        case 39:
