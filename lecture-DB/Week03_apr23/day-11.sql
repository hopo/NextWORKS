...
-- 1) 사원(employees)테이블에 전화번호(phone_number)에서
...
-- sol3
select phone_number,
    '(02)' || substr(phone_number, 5)
from employees ;


-- 2) 근속년수가 10년이상인 사원을, 근속년수 순서(내림차순) 조회
...
-- 5) 고객(customers)테이블에서 고객의 출생년(cust_year_of_birth)도
...
select cust_name,
    cust_year_of_birth,
    decode(
            trunc((to_number(to_char(sysdate, 'yyyy')) - cust_year_of_birth) / 10),
            3, '30대',
            4, '40대',
            5, '50대',
            '기타'
...
-- ALL and DISTINCT in count parameter
...
-- 분산(分散, variance) : variance()
-- 표준편차(標準 偏差, standard deviation) : stddev()
-- p155
