...
-- ### exA)
-- A부서 별, 총 급여 사원수를 조회해 주세요
select department_id, sum(salary), count(*)
from employees
group by department_id 
order by 1 ;

-- B부서 별, 업무 별, 총 급여 사원수를 조회해 주세요
...
-- ### ex2)
-- using rollup
-- A2.부서 별, 총 급여 사원수를 조회해 주세요 and 소계
select department_id, sum(salary), count(*)
from employees
...
-- using rollup
-- B2.부서 별, 업무 별, 총 급여 사원수를 조회해 주세요 and 각 엽무별 소계 부서별 소계
select department_id, job_id, sum(salary), count(*)
from employees
...
-- ### ex3)
-- ROLLUP을 사용하지 않고 동일한 조회
-- A3.부서 별, 총 급여 사원수를 조회해 주세요 and 소계
select department_id, sum(salary), count(*)
from employees
group by department_id
union
...
-- ROOLUP을 사용하지 않고
-- B3.부서 별, 업무 별, 총 급여 사원수를 조회해 주세요 and 각 엽무별 소계 부서별 소계
select department_id, job_id, sum(salary), count(*)
from employees
group by department_id, job_id
...
-- ### ex4)
-- 부분 소계
-- B4.부서 별, 총 급여 원수를 조회해 주세요  and 업무별로만 소계
select department_id, job_id, sum(salary), count(*)
from employees
group by department_id, rollup(job_id)
order by 1 ;

-- B4.부서 별, 총 급여 원수를 조회해 주세요  and 부서별로만 소계
select department_id, job_id, sum(salary), count(*)
from employees
group by rollup(department_id), job_id
order by 2 ;
...
-- EXAM)
-- 급여가 9000 이상인 사원을 조회
-- 사번(employee_id), 사원명(emp_name), 급여(salary),
-- 부서코드(department_id), 부서명(department_name)
