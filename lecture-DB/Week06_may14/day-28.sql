...
create or replace function fn_dept_name(p_dept_id number) return varchar2
is
...
-- function을 이용하여..
select employee_id, emp_name, department_id,
    fn_dept_name(department_id) -- 사용자 정의 함수
from employees a;
...
-- id157, yr1998 - query)
select sum(amount_sold) from sales where employee_id=157 and sales_month like '1998'||'%';
-- id157, yr1998 - query2)
select sum(amount_sold) from sales where employee_id = 157 and to_char(sales_date, 'YYYY') = '1998' ;
...
-- ### exB)
-- * fn_emp_amount()를 이용하여, 80번 부서의 사원들을 조회, 1998, 1999, 올해 매출액
select employee_id, emp_name, department_id,
