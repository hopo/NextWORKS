...
from employees
where (employee_id, department_id) in (select employee_id, department_id from job_history) ;
-- * 동시 만족

-- ### exC2) exC와 비교 (-> 비슷한 면은 있지만 의미는 다름)
select employee_id, emp_name, job_id, department_id
from employees
